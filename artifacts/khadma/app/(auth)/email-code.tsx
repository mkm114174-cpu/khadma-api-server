...
type Phase = "email" | "code" | "language" | "welcome" | "entering";
...
export default function EmailCodeScreen() {
  ...
  const verify = async () => {
    ...
    try {
      if (mode === "signIn") {
        ...
        if (verifyError) {
          setError(errorMessage(verifyError) ?? t.auth.invalidCode);
          return;
        }
      } else {
        ...
        const password = tempPasswordRef.current;
        if (!password) {
        ...
        if (signInError) {
          setError(errorMessage(signInError) ?? t.auth.loginFailed);
          return;
        }
      }
  ...
  }, [refresh, router, t.auth.finalizeFailed]);
      ...
      <View style={[styles.root, styles.center]}>
        {error ? (
          <>
            ...
            <Pressable style={styles.btn} onPress={() => void enterApp()}>
              <Text style={styles.btnText}>{t.layout.retry}</Text>
            </Pressable>
          </>
        ) : (
          <>
            <ActivityIndicator size="large" color={C.primary} />
            <Text style={styles.enteringText}>{t.auth.saving}</Text>
          </>
        )}
      </View>
    );
...
const makeStyles = (C: ReturnType<typeof useColors>) =>
  StyleSheet.create({
    ...
    enteringText: { color: C.mutedForeground, fontSize: 15 },
    scroll: {
      flexGrow: 1,
